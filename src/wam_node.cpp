...
// Function to hold WAM end effector Orientation -- better to use holdOrtn2 
template<size_t DOF>
...
bool WamNode<DOF>::forceTorqueBase(wam_srvs::ForceTorqueBase::Request &req, wam_srvs::ForceTorqueBase::Response &res) {
    ...
    //     cp_type Kth;
    //     cp_type Xd;
    //     Eigen::Quaterniond Thetad;
    //     Kx << 0.0, 0.0, 0.0;
    //     Dx << 1.0, 1.0, 1.0;
    //     Kth << 0.0, 0.0, 0.0;        
    //     Xd << wam.getToolPosition(); // e.g. position at [0,0,0,-pi/2,0,0,0] is [0.4 , 0 , 0.6]
    //     Thetad << wam.getToolOrientation(); // e.g. orientation at [0,0,0,-pi/2,0,0,0] is [-pi/2 , 0 , 0]
    //     KxSet.setValue(Kx);
    //     DxSet.setValue(Dx);
    //     KthSet.setValue(Kth);
    //     XdSet.setValue(Xd);
    ...
    //     return true;
    // } 
    // // else if (req.mode == 5) { // TOOL CARTESIAN CONNECT
    // //     // CONNECT CARTESIAN SPRING POSITION FOR ORIENTATION CONTROL
    // //     if (systems_connected) {
    // //         systems::disconnect(wam.input);
    // //         systems_connected = false;
    // //         ROS_INFO("ForceTorqueBase disconnected from mode 5");
    // //         btsleep(1.0);
    // //     }
    // //     // CONNECT FORCE TORQUE TOOL 
    // //     exposedOutputForce.setValue(ForceApplied);
    ...
    // //     systems::forceConnect(torqueSum.output, jtSat.input);
    // //     systems::forceConnect(jtSat.output, wam.input); 
    // //     ROS_INFO("ForceTorqueBase tool cartesian position connected from mode 5");
    // //     systems_connected = true;
    // //     return true;
    // // } else if (req.mode == 6) { // UPDATE SPRING VALUES
    // //     exposedOutputForce.setValue(ForceApplied);
    // //     exposedOutputTorque.setValue(TorqueApplied);
    // //     systems_connected = true;
    // //     return true;
